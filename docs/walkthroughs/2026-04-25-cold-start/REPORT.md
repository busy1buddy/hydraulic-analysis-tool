...
4. **Asymmetric bulk editing.** `Edit > Bulk Edit Pipes...` exists and is excellent (material picker, DN dropdown, roughness). There is no equivalent `Edit > Bulk Edit Junctions` for elevation or demand. To apply a uniform per-lot demand of 0.05 LPS to 13 junctions I had no GUI option — every junction would need its own dialog. *Fix:* mirror the Bulk Edit Pipes dialog for junctions — multi-select + bulk set elevation/demand/pattern.
...
6. **Menu organisation.**
   ...
   *Fix (low-risk):* deduplicate Settings and Pipe Profile. Group Analysis into three submenus: **Run** (steady, transient, EPS, fire flow, quality), **Configure** (slurry mode/params, demand patterns, water-quality config), **Reports** (compliance check, safety case, pump energy).
